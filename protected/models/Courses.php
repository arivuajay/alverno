...
class Courses extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('academic_yr_id, is_deleted', 'numerical', 'integerOnly'=>true),
			array('course_name, code, section_name', 'length', 'max'=>255),
			array('created_at, updated_at', 'safe'),
			array('course_name, code', 'required'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, course_name, code, section_name, academic_yr_id, is_deleted, created_at, updated_at', 'safe', 'on'=>'search'),
		);
	}
