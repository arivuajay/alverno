...
class Employees extends CActiveRecord
{
	...
	public function rules()
	{
		...
		if(Yii::app()->controller->action->id=='create2' or Yii::app()->controller->action->id=='update2'){
			return array(
				array('home_address_line1, home_city, home_state, home_country_id, home_pin_code, email, mobile_phone, qualification','required'),
				array('home_pin_code, office_pin_code, mobile_phone','numerical', 'integerOnly'=>true),
				array('email','check'),
				//array('email','unique'),
				array('mobile_phone','unique'),
				array('home_pin_code,office_pin_code','length', 'max'=>6),
				array('home_address_line1, home_address_line2, home_city, home_state, home_pin_code, office_address_line1, office_address_line2, office_city, office_state, office_pin_code, office_phone1, office_phone2, mobile_phone, home_phone, email, fax, photo_file_name, photo_content_type', 'length', 'max'=>255),
				array('email','email'),
		...
		return array(
			...
			array('employee_number, first_name, last_name, gender, date_of_birth, employee_department_id, qualification', 'required'),
   ...
   public function check($attribute,$params)
	...
	$parent= Guardians::model()->findByAttributes(array('email'=>$this->$attribute));
  
		if((Yii::app()->controller->action->id!='update' and Yii::app()->controller->id!='default') and $this->$attribute!='')
		{
			//$validate = User::model()->findByAttributes(array('email'=>$this->$attribute));
			
			if($student!=NULL or $parent!=NULL or ($employee!=NULL and $employee->id!=$this->id))
			{
