...
<div class="site_drrop">
   ...
   <?php  	if(ModuleAccess::model()->check('Transport')){ ?><li <?php if($setup_tra==true){?>class="completed"<?php }?>><?php echo CHtml::link(Yii::t('app','Transport'), array('/transport')); ?></li><?php } ?>
    ...
    <div class="sd_but_con">
		...
		echo CHtml::link(Yii::t('app','Students').'<span class="ex-small">'.count(Students::model()->findAll("is_deleted 	=:x", array(':x'=>0))).' '.Yii::t('app','Records').'</span><span class="act-btm"></span>',array('/site/manage','val' =>'A','widget'=>'s_a'),array('id'=>'main_tab_students','class'=>'active'));
			}
			else
			{
				echo CHtml::link(Yii::t('app','Students').'<span class="ex-small">'.count(Students::model()->findAll("is_deleted 	=:x", array(':x'=>0))).' '.Yii::t('app','Records').'</span><span class="act-btm"></span>',array('/site/manage','val' =>'A'),array('id'=>'main_tab_students','class'=>'active'));
			}
		...
		?>
	     ...
	     {
		 echo CHtml::link(Yii::t('app','Teachers').'<span class="ex-small">'.count(Employees::model()->findAll("is_deleted 	=:x", array(':x'=>0))).' '.Yii::t('app','Records').'</span><span class="act-btm"></span>',array('/site/emanage','val' =>'A'),array('id'=>'main_tab_teachers'));
