...
$this->breadcrumbs=array(
	...
	Yii::t('app','All Paid / Unpaid Students'),
...
window.location= 'index.php?r=fees/financeFees/allpaid&batch='+id;	
}
function category()
{
var id_1 = document.getElementById('batch').value;
var id = document.getElementById('category').value;
window.location= 'index.php?r=fees/financeFees/allpaid&batch='+id_1+'&course='+id;	
}
</script>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    ...
    <h1><?php echo Yii::t('app','All Paid / Unpaid Students');?></h1>
   ...
   <?php echo Yii::t('app','Fee Details');?>
    <table width="90%" cellspacing="0" cellpadding="0">
		...
		if(count($list)!='0'){ // Show SMS button only if unpaid students are present
			...
			?>
					<?php /*?><div class="edit_bttns" style="top:170px; right:340px;"> 
        ...
        <div class="ea_pdf" style="top:180px; right:10px;"><?php echo CHtml::link(Yii::t('app', 'Generate PDF'), array('/fees/FinanceFees/allpaidpdf','batch'=>$_REQUEST['batch'],'collection'=>$_REQUEST['course']),array('target'=>"_blank",'class'=>'pdf_but')); ?></div>
        <?php echo Yii::t('app','Students List');?>
        <br />
       <table width="90%" cellspacing="0" cellpadding="0">
        <tr>
         <th><strong><?php echo Yii::t('app','Sl no.');?> </strong></th>
	   ...
	   foreach($list as $list_1) {
		...
		{ 
		 if($list_1->is_paid == 0)
		 {
			...
			{
				echo CHtml::ajaxLink(Yii::t('app','Edit'), Yii::app()->createUrl('fees/FinanceFees/Editfees' ), array('type' =>'GET','data' =>array( 'id' => $list_1->id ),'dataType' => 'text',  'update' =>'#edit'.$list_1->id, 'onclick'=>'$("#editfees'.$list_1->id.'").dialog("open"); return false;',),array('class'=>'edit_fee',));
					echo '<div id="edit'.$list_1->id.'"></div>';
			}
			elseif($list_1->fees_paid>0)
			{
				echo CHtml::ajaxLink(Yii::t('app','Partial'), Yii::app()->createUrl('fees/FinanceFees/Partialfees' ), array('type' =>'GET','data' =>array( 'id' => $list_1->id ),'dataType' => 'text',  'update' =>'#partial'.$list_1->id, 'onclick'=>'$("#partialfees'.$list_1->id.'").dialog("open"); return false;',),array('class'=>'partial_fee',));
				echo '<div id="partial'.$list_1->id.'"></div>';
				echo ' | ';
				echo CHtml::link('Print Receipt',array('/fees/FinanceFees/partialreceipt','batch'=>$_REQUEST['batch'],'collection'=>$_REQUEST['course'],'id'=>$posts->id),array('target'=>'_blank')); 
		 ...
		 {
		 	echo CHtml::link(Yii::t('app','Print Receipt'),array('/fees/FinanceFees/printreceipt','batch'=>$_REQUEST['batch'],'collection'=>$_REQUEST['course'],'id'=>$posts->id),array('target'=>'_blank')); 
			echo ' | ';
				echo CHtml::ajaxLink(Yii::t('app','Edit'), Yii::app()->createUrl('fees/FinanceFees/Editfees' ), array('type' =>'GET','data' =>array( 'id' => $list_1->id ),'dataType' => 'text',  'update' =>'#edit'.$list_1->id, 'onclick'=>'$("#editfees'.$list_1->id.'").dialog("open"); return false;',),array('class'=>'edit_fee',));
					echo '<div  id="edit'.$list_1->id.'"></div>';
		 }
...
<script>
	$('.edit_fee, .partial_fee').click(function(e) {
		$('#partial-fees-form').remove();
	});
</script>
