	...
	else if(in_array($cell_date,$days) and !array_key_exists($cell_date, $holiday_arr))
	{
		if(($ac_year == $current_academic_yr->config_value) or ($ac_year != $current_academic_yr->config_value and $is_insert->settings_value!=0))
		{
                    echo CHtml::ajaxLink($span,$this->createUrl('StudentAttentance/addnew'),array(
                            'type' =>'GET','data'=>array('day' =>$day,'month'=>$month,'year'=>$year,'emp_id'=>$emp_id,'timetable' => $period->id),
                            'onclick'=>'$("#jobDialog'.$day.$emp_id.$pno.'").dialog("open"); return false;',
                            'update'=>'#jobDialog123'.$day.$emp_id.$pno,
                            ),array('id'=>'showJobDialog'.$day.'_'.$emp_id.'_'.$pno,'class'=>'at_abs'));
...
else
{
   ...
   #Column with leave marked
       echo CHtml::ajaxLink($span,$this->createUrl('StudentAttentance/EditLeave'),array(
        'onclick'=>'$("#jobDialog'.$day.$emp_id.$pno.'").dialog("open"); return false;',
        'update'=>'#jobDialogupdate'.$day.$emp_id.$pno,'type' =>'GET','data'=>array('id'=>$find[0]['id'],'day' =>$day,'month'=>$month,'year'=>$year,'emp_id'=>$emp_id),

        ),array('id'=>'showJobDialog'.$day.'_'.$emp_id.'_'.$pno,'title'=>Yii::t('app','Reason:').$find['0']['reason']));
