                ...
                <table width="100%" border="0" cellspacing="0" cellpadding="0">
				  ...
				  ?>
                    ...
                    <?php if(FormFields::model()->isVisible('batch_id','Students','forStudentProfile')){?>
                        <td align="center">
                        <?php
                        if($list_1->batch_id == 0)
                        {
                            $last_batch = BatchStudents::model()->findByAttributes(array('student_id'=>$list_1->id,'result_status'=>2));
                            $batch_id = $last_batch->batch_id; 
                        }
                        else
                        {
                            $batch_id = $list_1->batch_id;
                        }
                        $batc = Batches::model()->findByAttributes(array('id'=>$batch_id,'is_active'=>1,'is_deleted'=>0)); 
                        if($batc!=NULL)
                        {
