...
class StudentsController extends RController
{
	...
	public function actionUpdate($id,$status)
	{
		...
		{ 
			...
			{ 
				...
				// Saving to batch_student table to get current and previous batches of the student
				  if($model->batch_id and $model->batch_id!=NULL and $model->batch_id!='')
				  {
					  ...
					  if($old_batch_id != $model->batch_id)
					  {
					      ...
					      if($old_batch_id and $old_batch_id!=NULL and $old_batch_id!='')
						  {
						  ...
						  if($checkprevious and $checkprevious!=NULL)
						  {
	...
	public function actionDelete_student()
	{
		...
		if($model->saveAttributes(array('is_active'=>0, 'is_deleted'=>1)))
		{
			...
			$batch_students=BatchStudents::model()->findByAttributes(array('student_id'=>$id));
			if($batch_students)
			{
				$batch_students->saveAttributes(array('status'=>'0'));
			}
			...
			//remove guardian			
			$guardians = GuardianList::model()->findAllByAttributes(array('student_id'=>$_REQUEST['id']));
			if($guardians){						
			...
			/*$guardian_id = $model->parent_id;
			$model1 = Students::model()->findAllByAttributes(array('parent_id'=>$guardian_id, 'is_deleted'=>0));
			$count = count($model1);
			if($count<=1)
			{
				$guardian=Guardians::model()->findByPk($guardian_id);
			...
			$examscores = ExamScores::model()->DeleteAllByAttributes(array('student_id'=>$id));
		}
		...
		$datas = explode(",",$_REQUEST['id']);
			...
			foreach($datas as $data)
			{
				...
				if($model->saveAttributes(array('is_deleted'=>'1')))
				{
					...
					//remove guardian			
					$guardians = GuardianList::model()->findAllByAttributes(array('student_id'=>$data));
					if($guardians){						
						foreach($guardians as $guardian){
							$guardian_detail = Guardians::model()->findByAttributes(array('id'=>$guardian->guardian_id));
							if($guardian_detail!=NULL and $guardian_detail->uid != 0){
					...
					$batch_students=BatchStudents::model()->findByAttributes(array('student_id'=>$data));
					if($batch_students)
					{
						$batch_students->saveAttributes(array('status'=>'0'));
					}
	 ...
	 */
	 public function actionManage()
	 {
		...
		if(isset($_GET['print'])){
			...
			$html2pdf = Yii::app()->ePdf->HTML2PDF();
			$html2pdf->setDefaultFont('freesans');
			$html2pdf->WriteHTML($this->renderPartial('studentpdf', array('students'=>$students_list), true));
	...
	public function actionBatch()
	{
		...
		$data=Batches::model()->findAll('course_id=:id AND is_deleted=:x AND is_active=:y', 
                  array(':id'=>(int) $_POST['cid'],':x'=>'0',':y'=>1));
