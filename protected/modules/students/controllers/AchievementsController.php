			...
			if($model->user_id!='' and $model->doc_title!='' and $model->file!='' and $model->file_type!='') // Checking if Document name and file is uploaded
				{ 
				  ...
				  if(in_array($model->file_type,$valid_file_types)) // Checking file type
					{
					if($file_size <= 204800) // Checking file size
					{
					...
					{
						$flag = 0;
						Yii::app()->user->setFlash('errorMessage', Yii::t('app',"File size must not exceed 200kb!"));
					}
				  }
				  else
				  {
		...
		{ 
			...
			{ 
				...
				if($model->save())
				{
					if($_FILES['Achievements']['name']['file']!=NULL)
					{
						$old_destination_file = 'uploadedfiles/achievement_document/'.$model->user_id.'/'.$old_model['file'];	
				...
				else
				{
					...
					$this->redirect(array('update','id'=>$_POST['Achievements']['eid'],'document_id'=>$model->id,'model'=>$model));
				}
