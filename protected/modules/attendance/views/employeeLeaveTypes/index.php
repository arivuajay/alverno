                        ...
                        <table width="100%" cellpadding="0" cellspacing="0">
                            ...
                            if($active)
                            {
                                foreach($active as $active_1)
                                {
                                                                    ...
                                                                    if(($year == $current_academic_yr->config_value) or ($year != $current_academic_yr->config_value and $is_delete->settings_value!=0))
                                                                    {	
                                                                    echo '<td>'.CHtml::link(Yii::t('app','Delete'), array('delete', 'id'=>$active_1->id),array('confirm'=>Yii::t('app','Are you sure you want to delete this leave type?'))).'</td></tr>';
                                                                    }
                                }
                            }
                            else
                            ...
                            if($inactive)
                            {
                                foreach($inactive as $inactive_1)
                                {
                                    ...
                                    echo '<td>'.CHtml::link(Yii::t('app','Edit'), array('update', 'id'=>$inactive_1->id)).'</td>';
                                                                    ...
                                                                    if(($year == $current_academic_yr->config_value) or ($year != $current_academic_yr->config_value and $is_delete->settings_value!=0))
                                                                    {
                                    echo '<td>'.CHtml::link(Yii::t('app','Delete'), array('delete', 'id'=>$inactive_1->id),array('confirm'=>Yii::t('app','Are You Sure you want to delete the leave types ?'))).'</td></tr>';
                                                                    }
