...
<?php $form=$this->beginWidget('CActiveForm', array(
	...
	<div class="row">
        ...
        <?php 
			...
			{
			echo $form->dropDownList($model,'employee_leave_type_id',CHtml::listData(EmployeeLeaveTypes::model()->findAll("status=:x",array(':x'=>1)), 'id', 'name'),array('empty'=>Yii::t('app','Select Type'),'ajax' => array(
								'type'=>'POST',
								'url'=>CController::createUrl('/attendance/employeeAttendances/leaveCheck'),
								'update'=>'#leave_type',
			...
			{
				$leave_type = EmployeeLeaveTypes::model()->findByAttributes(array('id'=>$model->employee_leave_type_id));
				echo $form->textField($model,'employee_leave_type_id',array('value'=>$leave_type->name,'disabled'=>true));
			}
		?>
		<?php echo $form->error($model,'employee_leave_type_id'); ?>
	</div>
<div class="msg" id="leave_type" style="color:#F00"></div>
    <?php
	...
	<div class="row">
		...
		if(($ac_year == $current_academic_yr->config_value) or ($ac_year != $current_academic_yr->config_value and $is_edit->settings_value!=0))
		{
			echo $form->textField($model,'reason',array('size'=>30,'maxlength'=>45,'required'=>'required'));
		}
	...
	<div class="row halfday-con" id="halfday"  style="display:<?php echo $radio_display; ?>">
