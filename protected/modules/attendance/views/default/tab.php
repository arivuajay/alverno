 ...
 <?php    $batch=Batches::model()->findByAttributes(array('id'=>$_REQUEST['id'])); 
	...
	<div class="c_batch_tbar">
      <div class="edit_bttns" style="width:350px; top:-40px; right:-25px;">
            	<ul>
                <li>
                   <?php echo CHtml::link('<span>'.Yii::t('app','Teacher Attendance').'</span>',array('/attendance/employeeAttendances'),array('class'=>'addbttn','style'=>'top:-40px;'));?>
                   </li>
                 </ul>
    		</div>  
    <?php if((Yii::app()->controller->id=='studentAttentance' and Yii::app()->controller->action->id=='index')){
			
			echo CHtml::ajaxLink('<span>'.Yii::t('app','Change').' '.Yii::app()->getModule('students')->fieldLabel("Students", "batch_id").'</span>',array('/site/explorer','widget'=>'2','rurl'=>'attendance/studentAttentance'),array('update'=>'#explorer_handler'),array('id'=>'explorer_change','class'=>'sb_but','style'=>'right:40px; top:-40px;')); 	
           
			} 
			if(Yii::app()->controller->action->id=='batchwise'){
				echo CHtml::ajaxLink('<span>'.Yii::t('app','Change').' '.Yii::app()->getModule('students')->fieldLabel("Students", "batch_id").'</span>',array('/site/explorer','widget'=>'2','rurl'=>'/attendance/subjectAttendance/batchwise'),array('update'=>'#explorer_handler'),array('id'=>'explorer_change','class'=>'sb_but','style'=>'right:40px; top:-40px;'));
			}
            ...
            <?php echo CHtml::link('<span>'.Yii::t('app','close').'</span>',array('/attendance'),array('class'=>'sb_but_close','style'=>'right:0px;top:-40px;'));?>
