					...
					if(isset($_POST['recipients']) and $_POST['recipients']!=""){
						...
						if($instance->save()){
							...
							Yii::import('application.extensions.runactions.components.ERunActions');
       						ERunActions::touchUrl(Yii::app()->createAbsoluteUrl('/sms/sendsms/index',array('id'=>$instance->id)));
						...
						$response['message']	= Yii::t('app', 'Enter numbers to send SMS');
					}
				}
				else{
					$response['message']	= Yii::t('app', 'Enter message to send SMS');
				}				
			}
			else{
				$response['message']	= Yii::t('app', 'Invalid request');
			}
