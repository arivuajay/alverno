...
class EmployeeAttendancesController extends RController
{
	...
	public function actionAddnew() {
        ...
        {       
            ...
            if($model->save()) 
			{
				...
				ActivityFeed::model()->saveFeed(Yii::app()->user->Id,'26',$model->employee_id,ucfirst($employee->first_name).' '.ucfirst($employee->middle_name).' '.ucfirst($employee->last_name),$date,NULL,NULL);
				echo CJSON::encode(array(
                        'status'=>'success',
                        ));
                 exit;    
	...
	public function actionPdf1()
    {
		...
		$html2pdf = new HTML2PDF('L', 'A4', 'en');
                $html2pdf->setDefaultFont('freesans');
        $html2pdf->WriteHTML($this->renderPartial('empattentance', array(), true));
