...
class EmployeesController extends RController
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Employees']))
		{
			...
			*/
			...
			//if($model->save())
			//{
				//echo $list['joining_date'];exit;
				//echo $list['date_of_birth'];exit;
				...
				//Save the profile pic to the folder	
					if($model->photo_file_name!=NULL){
						...
						$destination_file = 'uploadedfiles/employee_profile_image/'.$model->id.'/'.$_FILES['Employees']['name']['photo_data'];
			...
			$model =$model->findByAttributes(array('id'=>$_REQUEST['id']));
				...
				if($model->save()){
						...
						if($user->save())
						{
							...
							$to = '';
							if($notification->sms_enabled=='1' and $notification->employee == '1') // Check if SMS is enabled
							{
								...
								if($to!='') // Send SMS if phone number is provided								
								{
									...
									$template=SystemTemplates::model()->findByPk(6);
									$message = $template->template;
									$message = str_replace("<School Name>",$college->config_value,$message);
									
									$template=SystemTemplates::model()->findByPk(7);
						...
						if($user->save())
						{
							...
							if($sms_settings[0]->is_enabled=='1' and $sms_settings[4]->is_enabled=='1'){ // Check if SMS is enabled
								...
								if($to!=''){ //If phone number is provided, send SMS
									$college=Configurations::model()->findByPk(1);
									$from = $college->config_value;
									$message = Yii::t('app','Welcome to')." ".$college->config_value;
									$login_message = Yii::t('app','Log on to')." ".$college->config_value." ".Yii::t('app','account with your email as username and')." ".$password." ".Yii::t('app','as password.');
									SmsSettings::model()->sendSms($to,$from,$message);
									SmsSettings::model()->sendSms($to,$from,$login_message);
								} // End send SMS
							} // Check if SMS is provided
							...
							UserModule::sendMail($model->email,Yii::t('app',"You are registered from)"."{site_name}",array('{site_name}'=>Yii::app()->name)),Yii::t('app',"Please login to your account with your email id as username and password"."{password}",array('{password}'=>$password)));
						}
    ...
    {
		$employee = Employees::model()->findByAttributes(array('id'=>$_REQUEST['id']));
		$employee = $employee->first_name.' '.$employee->last_name.' Subjects.pdf';
        $html2pdf = Yii::app()->ePdf->HTML2PDF();
        $html2pdf->setDefaultFont('freesans');
		$html2pdf->WriteHTML($this->renderPartial('printsubjects', array('model'=>$this->loadModel($_REQUEST['id'])), true));
        $html2pdf->Output($employee);
	...
	public function actionManage()
	 {
	   ...
	   if((substr_count( $_REQUEST['name'],' '))==0)
		...
		if(isset($_GET['print'])){
			...
			$html2pdf = Yii::app()->ePdf->HTML2PDF();
			$html2pdf->setDefaultFont('freesans');
			$html2pdf->WriteHTML($this->renderPartial('employeepdf', array('employee'=>$posts), true));
			$html2pdf->Output($receipt);
		}
	 ...
	 public function actionDeletes()
	{
		...
		if($model->saveAttributes(array('is_deleted'=>'1')))
		{
			       if($user=User::model()->findByAttributes(array('id'=>$model->uid)))
					...
					$timetables=TimetableEntries::model()->findAllByAttributes(array('employee_id'=>$model->id));
					...
					if($timetables)
					{
						foreach($timetables as $timetable)    // delete asociated timetables
						{
							$timetable->delete();
						}
					}
