...
class CalWidget extends CWidget
{
    public function run()
    {
        ...
        Yii::getPathOfAlias('application.modules.holidays.components.fullCal') . '/dbfullcalendar.css'
    ));
	
	$cs->registerCssFile(Yii::app()->assetManager->publish(
        Yii::getPathOfAlias('application.modules.holidays.components.fullCal') . '/eventCal.css'
    ));
      */  
	...
	$cs->registerScriptFile(Yii::app()->assetManager->publish(
        Yii::getPathOfAlias('application.modules.holidays.components.fullCal') . '/fullcalendar.js'
    ));
	
	$cs->registerScriptFile(Yii::app()->assetManager->publish(
        Yii::getPathOfAlias('application.modules.holidays.components.fullCal') . '/eventCal.js'
    ));
       ...
       // $cs->registerScriptFile($scriptUrl .'/js/jquery-ui-1.8.17.custom.min.js');
        ...
        $param['baseUrl']= Yii::app()->createUrl('holidays/main').'/';
        $param['newEventPromt']=Yii::t('HolidaysModule.fullCal', 'New Holiday:');
    ...
    /*function translateArray($arr)
    {
        foreach ($arr as $key => $data)
        {
            ...
            {
                foreach ($data as $k => $d)
                    $data[$k] = Yii::t('HolidaysModule.fullCal', $d);
                $arr[$key] = $data;
            }
            else
                $arr[$key] = Yii::t('HolidaysModule.fullCal', $data);
        }
        return $arr;
    }*/
}
