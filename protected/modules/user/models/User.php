...
class User extends CActiveRecord
{
	...
	public function relations()
	{
        ...
        {
            $student=NULL; $employee=NULL; $guardians=NULL;
		...
		foreach($roles as $role)
		{
                ...
                {                   
                    ...
                    $criteria->condition= 'uid<>:uid AND email=:email AND is_delete=0';
                    $criteria->params=array(':uid'=>$this->id,':email'=>$this->email);                    
                    $guardians= Guardians::model()->findAll($criteria); 
                }
                if($user_base!='teacher')
                {
                    $criteria= new CDbCriteria();
                    $criteria->condition= 'uid<>:uid AND email=:email AND is_deleted=0';
                    $criteria->params=array(':uid'=>  $this->id,':email'=>$this->$attribute);                    
                    $employee= Employees::model()->findAll($criteria);
                    
                }
                if($user_base!='student')
                {
                    $criteria= new CDbCriteria();
                    $criteria->condition= 'uid<>:uid AND email=:email';
                    $criteria->params=array(':uid'=>  $this->id,':email'=>$this->$attribute);                       
		...
		if($this->$attribute!='')
		{
			if($student!=NULL or $employee!=NULL or $guardians!=NULL)
			{
