...
<?php $batch=Batches::model()->findByAttributes(array('id'=>$_REQUEST['id'])); ?>
		...
		if($course!=NULL)
          ...
          <?php if((isset($_REQUEST['id']) and $_REQUEST['id']!=NULL) and ((Yii::app()->controller->id=='weekdays' and (Yii::app()->controller->action->id=='index' or Yii::app()->controller->action->id=='timetable')) or (Yii::app()->controller->id=='classTiming' and Yii::app()->controller->action->id=='index')))
			{
				?>
