...
class Employees extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('employee_number, joining_date, first_name, last_name, gender, email, home_address_line1,  home_city, home_state, home_pin_code, mobile_phone', 'required'),
			array('experience_year, experience_month', 'exp_validation'),
			array('email, employee_number','unique'),
			array('email','email'),
			array('email','check'),
			/*validation rules for importcsv*/
			
			array('employee_category_id, employee_position_id, employee_department_id, reporting_manager_id, employee_grade_id, experience_year, experience_month, children_count, nationality_id, home_country_id, home_pin_code, office_country_id, office_pin_code, photo_file_size, is_deleted, user_id, uid, office_phone1, office_phone2, mobile_phone, home_phone', 'numerical', 'integerOnly'=>true),
			array('employee_number, gender, first_name, middle_name, last_name, job_title, employee_number,qualification, status_description, marital_status, father_name, mother_name, husband_name, blood_group', 'length', 'max'=>255),
   ...
   public function check($attribute,$params)
    {
		if((Yii::app()->controller->action->id!='update' and Yii::app()->controller->id!='default') and $this->$attribute!='' and Yii::app()->controller->action->id!='update2')
		{
   ...
   public function getConcatened()
	{
	 ...
	 public function number_check($attribute,$params)
    {
		if((Yii::app()->controller->action->id!='update' and Yii::app()->controller->id!='default') and $this->$attribute!='' and Yii::app()->controller->action->id!='update2')
		{
			$validate = Employees::model()->findByAttributes(array('mobile_phone'=>$this->$attribute));
			$validate1 = Students::model()->findByAttributes(array('phone1'=>$this->$attribute));
			if($validate!=NULL or $validate1!=NULL)
			{
			
				$this->addError($attribute,Yii::t('app',Yii::t('app','Mobile number already in use')));
			}
		}
    }
}