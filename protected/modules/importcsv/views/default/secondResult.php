...
elseif($error==0){
    ...
    $thirdContent = '<input type=\"hidden\" name=\"Tablekey\" value=\"\"/><input type=\"hidden\" name=\"CSVkey\" value=\"\"/><input type=\"hidden\" name=\"hmodel\" id=\"hmodel\" value=\"'.$selectedmodel.'\"/><input type=\"hidden\" name=\"perRequest\" id=\"perRequest\" value=\"'.$paramsArray['perRequest'].'\"/><div class=\"formCon\"><div class=\"formConInner\"><table  cellpadding=\"0\" cellspacing=\"0\" border=\"0\" width=\"100%\"><tr><th align=\"left\">'.Yii::t('app', 'Table column').'</th><th align=\"left\">'.Yii::t('app', 'CSV column').'</th></tr><tr><th>&nbsp;</th><th>&nbsp;</th></tr>';
    ...
    for($i=0; $i<$length; $i++) {
        ...
        $thirdContent = $thirdContent.'<tr><td height=\"50\">'.$selectedmodel::model()->getAttributeLabel($tableColumns[$i]).$reqfield.'</td><td><select name=\"Columns['.$i.']\" id=\"select_'.$i.'\">'.$optionsContent.'</select></td></tr>';		
    }
	...
	if(isset(Yii::app()->controller->module->models[$selectedmodel]['external'])){
		...
		foreach($external_entry as $field=>$entry){			
			...
			foreach($entry['allowedColumns'] as $key=>$column){
				...
				$thirdContent = $thirdContent.'<tr><td height=\"50\">'.$extmodel::model()->getAttributeLabel($column).$reqfield.'</td><td><select name=\"ExtColumns['.$field.']['.$column.']\" id=\"select_ext_'.$key.'\">'.$optionsContent.'</select></td></tr>';
			}
			...
			if(isset($entry['compare']))
			{
				...
				foreach($ext_compare_entry as $extfield=>$extentry){	
					...
					//model title
					$thirdContent .= '<tr><td colspan=\"2\"><h3 style=\"font-size:14px; color:#777;\">'.$extentry['label'].'</h3></td></tr>';
					
					//model description
					if(isset($entry['description'])){
						$thirdContent .= '<tr><td colspan=\"2\"><span style=\"font-size:10px; color:#777;\">'.$extentry['description'].'</span></td></tr>';
					}
	...
	//for compare entries
	if(isset(Yii::app()->controller->module->models[$selectedmodel]['compare'])){
		$compare_entry	=	Yii::app()->controller->module->models[$selectedmodel]['compare'];
		foreach($compare_entry as $field=>$entry){			
			...
			foreach($entry['allowedColumns'] as $key=>$column){
				...
				$thirdContent = $thirdContent.'<tr><td height=\"50\">'.$extmodel::model()->getAttributeLabel($column).$reqfield.'</td><td><select name=\"CompColumns['.$field.']['.$column.']\" id=\"select_cmp_'.$key.'\">'.$optionsContent.'</select></td></tr>';
			}
		}
	}
