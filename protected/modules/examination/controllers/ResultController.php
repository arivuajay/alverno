...
class ResultController extends RController
{
	...
	public function actionIndex()
	{
		...
		if($_GET['search_id']=='1' and isset($_GET['search_button']))
		{
			...
			{ 
				$flag=1;
				$batch = $_GET['batch_id'];
				$students = Students::model()->findAllByAttributes(array('batch_id'=>$_GET['batch_id']));
	...
	public function actionResultpdf()
    {
		...
		$pdf_name = 'Rsult Search'.' Report.pdf';
        # HTML2PDF has very similar syntax
        $html2pdf = Yii::app()->ePdf->HTML2PDF();
        $html2pdf->setDefaultFont('freesans');
		
		ob_end_clean();
        $html2pdf->WriteHTML($this->renderPartial('resultpdf', array('search_id'=>$_REQUEST['search_id'],'course'=>$_REQUEST['course'],'batch'=>$_REQUEST['batch'],'group'=>$_REQUEST['group'],'exam'=>$_REQUEST['exam']), true));
        $html2pdf->Output($pdf_name);
