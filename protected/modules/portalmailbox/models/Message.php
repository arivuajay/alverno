...
class Message extends CActiveRecord
{
	...
	{
		return PortalmailboxModule::TBL_MSG;
	}
	...
	{
		$module =& Yii::app()->getModule('portalmailbox');
		$p = new CHtmlPurifier();
		if(Yii::app()->getModule('portalmailbox')->isAdmin())
	...
	public function sent($userid)
	{
		...
		$this->getDbCriteria()->mergeWith(array(
			...
			'alias'=>'m',
			'join'=>"
			INNER JOIN ".PortalmailboxModule::TBL_CONV."  AS c ON(c.conversation_id=m.conversation_id)
			LEFT JOIN (
				SELECT conversation_id, created FROM ".PortalmailboxModule::TBL_MSG." 
		...
		$is_replied = "
			INNER JOIN (
				SELECT conversation_id, IF(sender_id=:userid,'yes','no') AS is_replied FROM ".PortalmailboxModule::TBL_MSG." 
