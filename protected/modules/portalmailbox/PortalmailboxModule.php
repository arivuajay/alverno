 ...
 *	Iceweasal 3.5+
 */
class PortalmailboxModule extends CWebModule
{
	/*
	...
	*/
	...
	public function init()
	{
		...
		$this->setImport(array(
			'portalmailbox.models.*',
			'portalmailbox.components.*',
		));
	...
	*/
	public function autoComplete($term)
	{
		...
		foreach($users as $user)
		{
			...
			$json .= '{"label":"'.$user->{$this->emailColumn}.'",'
	...
	{
		$list = array('admin'=>Yii::t('app','Site Administrator'),'support'=>Yii::t('app','Customer Support'),'billing'=>Yii::t('app','Billing Administrator') );
		
		// we add site news as an option for the admin to create news updates by messaging the news box...
 		if($this->isAdmin())
			$list[$this->getUserName($this->newsUserId)] = Yii::t('app','Site News');
		return $list;
	}
...
EOD;
		...
		$this->_cs->registerScript('mailbox-js',$js,CClientScript::POS_END);
...
EOD;
	...
	*/
	public function getAssetsUrl()
	{
		...
		{
			$assetsPath = Yii::getPathOfAlias('portalmailbox.assets');
