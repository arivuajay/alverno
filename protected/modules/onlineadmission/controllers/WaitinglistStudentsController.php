...
class WaitinglistStudentsController extends RController
{
	...
	public function actionList()
	{
				...
				$criteria->params = array(':status' => 0,':st_Status' => -3,':st_academic_yr' => $online_academic_yr->config_value);				
				$criteria->order = 'st.last_name ASC';
				/*if(isset($_REQUEST['name']) and $_REQUEST['name']!=NULL)
				{									 
				 $criteria->condition=$criteria->condition.' and '.'(st.first_name LIKE :name)';
				 $criteria->params[':name'] = $_REQUEST['name'].'%';
				 				
				}*/
				if(isset($_REQUEST['val']))
				{
				 $criteria->condition=$criteria->condition.' and '.'(st.first_name LIKE :match or st.last_name LIKE :match or st.middle_name LIKE :match)';
		...
		$this->render('list',array('waitingListStudents'=>$waitingListStudents,'model'=>$model,'item_count'=>$total,
										'pages'=>$pages,										
										'criteria'=>$criteria,));
	...
	public function actionManage()
	{
		...
		{			
			...
			if($model->save())
			{
				//$student->saveAttributes(array('batch_id'=>$_POST['WaitinglistStudents']['batch_id']));	
	...
	public function actionDelete()
	{
		$model = WaitinglistStudents::model()->findByAttributes(array('student_id'=>$_REQUEST['id']));	
		$student = Students::model()->findByAttributes(array('id'=>$_REQUEST['id']));
		//Fetch list of all students in this batch
		$criteria = new CDbCriteria;
