	...
	public function actionApproval()
	{
		$model=new Students;
			...
			if(ModuleAccess::model()->check('Students'))
			{
				...
				$online_academic_yr = OnlineRegisterSettings::model()->findByAttributes(array('id'=>2));
				...
				$criteria->params = array(':status'=>0,':is_completed'=>3,':academic_yr'=>$online_academic_yr->config_value);
	...
	public function actionOnlineApplicants()
	{
		...
		foreach($roles as $role)
		{
			...
			if(ModuleAccess::model()->check('Students'))
			{
				...
				//$criteria->params[':is_deleted'] = 0;
				$criteria->params = array(':is_deleted'=>0,':is_completed'=>3,':academic_yr'=>$online_academic_yr->config_value,'is_online'=>1);
			//Filter Section Conditions
				if(isset($_REQUEST['val']))
				{
				 $criteria->condition=$criteria->condition.' and '.'(first_name LIKE :match or last_name LIKE :match or middle_name LIKE :match)';
...
//view the profile details of student & parent 	
	...
	*/
	public function actionDelete()
	{
		...
		$model->delete();
		$guardian_list = GuardianList::model()->findByAttributes(array('student_id'=>$_REQUEST['id']));
		if($guardian_list){
			$guardian_list->delete();
		}
	//Deleting student documents	
		$studentDocuments = StudentDocument::model()->findAllByAttributes(array('student_id'=>$_REQUEST['id']));
	...
	//Deleting waiting list datas if the student is in waiting list
		$waitinglist = WaitinglistStudents::model()->findByAttributes(array('student_id'=>$_REQUEST['id']));
		if($waitinglist!=NULL)
		{
			$criteria = new CDbCriteria;
	...
	{
		$path = 'uploadedfiles/student_document/demo';
