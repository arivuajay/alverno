...
if(count($find)==0)
{
	...
	else if(in_array($cell_date,$days) and !array_key_exists($cell_date, $holiday_arr))
	{
		...
		echo CHtml::ajaxLink('ll',$this->createUrl('/teachersportal/default/addnew'),array(
				...
				'update'=>'#jobDialog123'.$day.$emp_id,'type' =>'GET','data'=>array('day' =>$day,'month'=>$month,'year'=>$year,'emp_id'=>$emp_id,'batch_id'=>$batch_id),
    ...
    <?php
	 ...
	 if($leave_types!=NULL)
	 {
	 echo CHtml::ajaxLink('<span class="abs1" style="color:'.$leave_types->colour_code.';text-align:center;padding-top:2px">'.$leave_types->label.'</span>',$this->createUrl('/teachersportal/default/EditLeave'),array(
        'onclick'=>'$("#jobDialog'.$day.$emp_id.'").dialog("open"); return false;',
        'update'=>'#jobDialogupdate'.$day.$emp_id,'type' =>'GET','data'=>array('id'=>$find[0]['id'],'day' =>$day,'month'=>$month,'year'=>$year,'emp_id'=>$emp_id,'batch_id'=>$batch_id),
		
        ),array('id'=>'showJobDialog'.$day.$emp_id,'title'=>'Reason: '.$find['0']['reason']));
	 }
	 else
	 {
		 echo CHtml::ajaxLink('<span class="abs" style="color:'.$leave_types->colour_code.';text-align:center;padding-top:2px">'.$leave_types->label.'</span>',$this->createUrl('/teachersportal/default/EditLeave'),array(
        'onclick'=>'$("#jobDialog'.$day.$emp_id.'").dialog("open"); return false;',
        'update'=>'#jobDialogupdate'.$day.$emp_id,'type' =>'GET','data'=>array('id'=>$find[0]['id'],'day' =>$day,'month'=>$month,'year'=>$year,'emp_id'=>$emp_id,'batch_id'=>$batch_id),
		
        ),array('style'=>'margin:0px','id'=>'showJobDialog'.$day.$emp_id,'title'=>'Reason: '.$find['0']['reason']));
