    ...
    <div class="edit_bttns" style=" float:right">
        <ul>
			...
			if(Yii::app()->controller->action->id=='allexams'){
				...
				$scheduleUrlExp = 'Yii::app()->createUrl("/teachersportal/exams/allexamschedule",array("bid"=>$data->batch_id,"exam_group_id"=>$data->id))';
				$resultUrlExp = 'Yii::app()->createUrl("/teachersportal/exams/allexamresult",array("bid"=>$data->batch_id,"exam_group_id"=>$data->id))';
			}			
			?>
            <li><span>
        	<?php echo CHtml::link('<span>'.Yii::t('examination','Change').' '.Yii::app()->getModule('students')->fieldLabel("Students", "batch_id").'</span>', array('/teachersportal/exams/allexam'),array('id'=>'add_exam-groups','class'=>'addbttn')); ?>
        	</span></li>
        </ul>
        <div class="clear"></div>
    </div>
