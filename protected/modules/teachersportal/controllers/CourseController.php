...
class CourseController extends RController
{
	...
	public function actionIndex()
	{
		...
		$criteria->join = 'INNER JOIN batches ON t.id = batches.employee_id' ;
		$criteria->distinct = true;
		$criteria->condition='uid = :match4';
		$criteria->params[':match4'] = Yii::app()->user->id;
		$classteacher= Employees::model()->find($criteria);
		$batch_array=array();
		if(isset($classteacher) && $classteacher!= NULL)
		{
			$coursearray1=Batches::model()->findAllByAttributes(array('employee_id'=>$classteacher->id));
			foreach($coursearray1 as $coursearray12)
			{
				if(!in_array($coursearray12->id,$batch_array))
				$batch_array[]=$coursearray12->id;
			}
