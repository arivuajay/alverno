...
public function actionSettime()
	{
	  ...
	  if(isset($_POST['TimetableEntries'])){ //var_dump($_POST);exit;
			   $model->attributes=$_POST['TimetableEntries'];
                           $errors= array();
			   ...
			   if($model->validate()) 
				   ...
				   if(isset($_POST['elective_id']) && $_POST['elective_id']!= NULL)
					   ...
					   foreach($_POST['elective_id'] as $key => $val)
                                           {                                                                                                                                                                                            
						   $elective_model = new TimetableEntries;
	...
	public function actionEmployeelist()
	{
		...
		$is_elective = 0;
		$timetable = $_POST['TimetableEntries'];
		if($timetable['subject_id'])
		{
			$subject = Subjects::model()->findByAttributes(array('id'=>(int) $timetable['subject_id']));
			if($subject->elective_group_id!=0) // Elective Selected
			{
				echo 'elective';
				$electives = Electives::model()->findAllByAttributes(array('elective_group_id'=>(int) $subject->elective_group_id));
				echo CHtml::openTag('tr');
				echo CHtml::tag('td', array(), CHtml::label(Yii::t('app','Teacher')).'<br/>');
				...
				foreach($electives as $elective)
				{
					$options = '';
					echo CHtml::openTag('tr');
						echo CHtml::tag('td', array(), '<h5 style="margin:10px;">'.ucfirst($elective->name).'</h5>');
						$data=EmployeeElectiveSubjects::model()->findAll('subject_id=:sub_id and elective_id=:elective_id',array(':sub_id'=>(int) $timetable['subject_id'],':elective_id'=>$elective->id));
						$options .= CHtml::tag('option', array('value' => ''), CHtml::encode('Select Teacher'), true);
						$data=CHtml::listData($data,'id','employee_id');
						foreach($data as $value=>$name)
						{
							$emp_name = Employees::model()->findByAttributes(array('id'=>$name));
							$options .= CHtml::tag('option',array('value'=>$emp_name->id),$emp_name->first_name.' '.$emp_name->last_name,true);
						}
						$dropdown = CHtml::tag('select',array('name'=>'TimetableElectiveEntries[employee_id]','id'=>'TimetableElectiveEntries_employee_id'),$options,true);
						echo CHtml::tag('td', array(), $dropdown);
					echo CHtml::closeTag('tr');
					echo CHtml::openTag('tr');
				...
				//$options .= CHtml::tag('option', array('value' => ''), CHtml::encode('Select Employee'), true);
			...
			else // Normal subject
			{
				...
				$data=EmployeesSubjects::model()->findAll('subject_id=:sub_id',array(':sub_id'=>(int) $timetable['subject_id']));
				echo CHtml::openTag('tr');
				echo CHtml::tag('td', array(), CHtml::label('Teacher'));
				
				$options .= CHtml::tag('option', array('value' => ''), CHtml::encode(Yii::t('app','Select Teacher')), true);
				$data=CHtml::listData($data,'id','employee_id');
				foreach($data as $value=>$name)
				{
					$emp_name = Employees::model()->findByAttributes(array('id'=>$name));
				...
				$dropdown = CHtml::tag('select',array('name'=>'TimetableEntries[employee_id]','id'=>'TimetableEntries_employee_id'),$options,true);
				echo CHtml::tag('td', array(), $dropdown);
				echo CHtml::closeTag('tr');
	...
	{	
		...
		if($subject->elective_group_id != 0){
			...
			if($group){
				$electives =  Electives::model()->findAllByAttributes(array('elective_group_id'=>$group->id,'batch_id'=>$_REQUEST['batch_id'],'is_deleted'=>0));
				...
				throw new CHttpException(404,Yii::t('app','The specified post cannot be found.'));
		}else{
		
			echo json_encode(array('status'=>'Nonelective', 'data'=>$this->renderPartial('_ajax_dropdown',
														array('subject_id'=>$_REQUEST['sub_id']),true,false)));
			Yii::app()->end();
	...
	public function actionRemove($id)
	{
		...
		$this->redirect(array('/courses/weekdays/timetable','id'=>$_REQUEST['batch_id']));
	}
	...
	/*public function actionCheckEmployee()
		{
		$class=$taken=TimetableEntries::model()->findAllByAttributes(array('employee_id'=>$_REQUEST['employee_id'],'id'=>$_REQUEST['class_timing_id'],'weekday_id'=>$_REQUEST['weekday_id']));
		if(count($class)>1)
		{
