...
if(isset($_REQUEST['id']))
{
	...
	//$criteria->condition = 'batch_id=:batch AND is_deleted=:is_deleted AND is_active=:is_active';
	$criteria->condition = 'is_deleted=:is_deleted AND is_active=:is_active';
	//$criteria->params[':batch'] = $_REQUEST['id'];
...
?>
	...
	if($year != $current_academic_yr->config_value and ($is_insert->settings_value==0 or $is_insert->settings_value==0 or $is_edit->settings_value==0 or $is_delete->settings_value==0))
	{
	?>
		<span style="color:#008000; font-size:12px; position:absolute; top:57px; left:150px;"> 
			<?php echo Yii::t('app','You are not viewing the current active year.'); ?>
			<?php 
			...
			elseif($is_insert->settings_value!=0 and $is_edit->settings_value!=0 and $is_delete->settings_value==0)
			{
				echo Yii::t('app','To delete the scores, enable Edit option in Previous Academic Year Settings.'); 
			}
			else
			{
            ...
            ?>
                ...
                <div class="tableinnerlist">
								...
								<tr>
                                    ...
                                    <td>
                                        ...
                                        if($sub->elective_group_id!=0)
                                        {
                                            ...
                                            //{
                                            ?>
                                                <?php echo '<i><span style="color:#E26214;">'.Yii::t('app','Elective not assigned').'</span></i>  '.CHtml::link(Yii::t('app','Add now'),array('/courses/batches/elective','id'=>$_REQUEST['id'])); ?>
                                            <?php
                                           // }
										  $flag=1;
                                        }
                                       ...
                                       if(FormFields::model()->isVisible("fullname", "Students", "forStudentProfile"))
                                        { ?>
                                            <td>                       
                                                ...
                                                echo $name;
                                                //echo $posts_1->first_name.' '.$posts_1->middle_name.' '.$posts_1->last_name;?>
                                            </td>
                                        <?php } ?>
                                            <td>
                                        	<?php
                    ...
                    if($i==1 and $checksub!=NULL)
                    {
                        ...
                        echo '<div class="notifications nt_green">'.Yii::t('app','Class Average').' = '.number_format($avg,2).'</div>';
                ...
                //   <!-- DYNAMIC FIELD ARRAY START -->    
                        ...
                        if(FormFields::model()->isVisible("fullname", "Students", "forStudentProfile"))
                        {
                            $new_array[]=array(
                                            ...
                                            'value'=>'$data->gridStudentName(forStudentProfile)',                                                                                        
                                            'name'=> 'firstname',
                                            'sortable'=>true,
                                    );
                        }
                        $new_array[]= array(
						'header'=>Yii::t('app','Marks'),
                        'value'=>'number_format($data->marks)',
                        'name'=>'marks',
                        );
                        $new_array[]= 'remarks';
                        $new_array[]= array(
