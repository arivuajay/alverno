...
<div class="sent_table_image"></div>
    ...
    <form id="select-contact-form">
        ...
        <td>
			...
			?>
            <ul>
                <li><input type="checkbox" id="check-all-contacts" value="" /> <label for="check-all-contacts"><?php echo Yii::t('app', 'Select All');?></label></li>
            </ul>
            <?php
			}			
			else{
            ...
            <ul>
                <li><input type="checkbox" value="<?php echo $contact->id;?>" data-name="<?php echo $contact->first_name.' '.$contact->last_name;?>" data-mobile="<?php echo $contact->mobile;?>" data-email="<?php echo $contact->email;?>" name="contact[]" id="contact-<?php echo $contact->id;?>" class="select_contact" /> <label for="contact-<?php echo $contact->id;?>"><?php echo $contact->first_name.' '.$contact->last_name.':'.$contact->email?></label></li>
            </ul>
            </div>
            <?php
            }
...
$('#add-contacts-to-send-box').click(function(e) {
	...
	sel_contacts.each(function(index, element) {
		var name	= $(element).attr('data-name'),
			mobile	= $(element).attr('data-email');
		value		= name + ':' + mobile;		
