...
<div class="sent_table_image_left"></div>
    <form id="select-group-form">
        ...
        <td>
			...
			?>
			<div style="padding-top:10px; width:94%; margin-top:10px;" class="notifications nt_red"><i><?php echo Yii::t('app','No groups found').'. '.CHtml::link(Yii::t('app','Create Now'), array('/sms/contactgroups/create'), array('target'=>'_blank'));?></i></div>
			<?php
			}
			?>
            </div>
...
$('#add-groups-to-send-box').click(function(e) {
	...
	if(groups.length){
		$.ajax({
			url:'<?php echo Yii::app()->createUrl('/sms/contactgroups/emailcontacts');?>',
			type:'POST',
			data:{groups:groups},
			cache:false,
			dataType:"json",
