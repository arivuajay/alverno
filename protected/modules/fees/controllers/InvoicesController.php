...
class InvoicesController extends RController
{
	...
	public function actionIndex(){
		...
		//all invoices
		$total_invoices		= FeeInvoices::model()->count();
		...
		$this->render('index', array('search'=>$search, 'invoices'=>$invoices, 'pages' => $pages, 'item_count'=>$total, 'total_invoices'=>$total_invoices, 'page_size'=>$page_size, 'paid_invoices'=>$paid_invoices));
	}
	...
	public function actionEdit(){
		...
		if(Yii::app()->request->isAjaxRequest){
			$id 		= $_POST['invoice_id'];
			if(isset($_POST['FeeInvoiceParticulars']['name'])){	//save
				...
				foreach($_POST['invoice-check'] as $invoice_id){
					...
					if($invoice!=NULL){
						if($invoice->user_type==1){							
							...
							$criteria->compare('`gl`.`student_id`', $student_id);
							...
							foreach ($guardians as $key => $guardian) {
								if($notification->mail_enabled == 1 and $guardian->email!=NULL){
									...
									$message 	= str_replace("{{FEE DUE DATE}}", ($date_config!=NULL)?date($date_config->displaydate, strtotime($invoice->due_date)):$invoice->due_date, $message);
	...
	public function actionPrint($id){
		...
		if($invoice){
			...
			$criteria->compare('invoice_id', $transaction->invoice_id);
			$alltransactions	= FeeTransactions::model()->findAll($criteria);
			//$this->renderPartial('_print', array('invoice'=>$invoice, 'particulars'=>$particulars, 'alltransactions'=>$alltransactions));exit;
