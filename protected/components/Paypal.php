...
class Paypal extends CComponent{
    ...
    */
    ...
    public function init(){
		...
		$paypalconfig         = PaypalConfig::model()->findByPk(1);
		$this->apiUsername    = $paypalconfig->apiusername;
		$this->apiPassword    = $paypalconfig->apipassword;
		$this->apiSignature   = $paypalconfig->apisignature;        
        $this->currency       = $paypalconfig->apicurrency;
		...
		if((bool)$this->apiLive === true){
            ...
            $this->endPoint = 'https://old-api-3t.sandbox.paypal.com/nvp';
        }
	
        //set return and cancel urls
		
		if(Yii::app()->session['signup_id'])
		{
			$this->returnUrl = Yii::app()->createAbsoluteUrl('paypal/signupConfirm/');
    ...
    public function SetExpressCheckout($paymentInfo=array()){ 
        ...
        $nvpstr='&AMT='.$amount.'&PAYMENTACTION='.$paymentType.'&CURRENCYCODE='.$currencyCode.'&RETURNURL='.$returnURL.'&CANCELURL='.$cancelURL.'&DESC='.$desc.'&QTY='.$quantity.'&SOLUTIONTYPE=Sole&LANDINGPAGE=Billing';
